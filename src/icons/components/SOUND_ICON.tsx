...
const SOUND_ICON: FC<IconProps> = ({className, color, innerKey}) => {
  const firstLine = useMemo(() => <line key={`cross-one-${innerKey}`} data-testid='clicked' className="cls-1" x1="228.4"
                                        y1="0.6" x2="0.6" y2="228.4" stroke="#a7000d" strokeWidth='20px'/>, [innerKey])
  const secondLine = useMemo(() => <line key={`cross-two-${innerKey}`} className="cls-1" x1="0.6" y1="0.6" x2="228.4"
                                         y2="228.4" stroke="#a7000d" strokeWidth='20px'/>, [innerKey]);
